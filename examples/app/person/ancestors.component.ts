...
@Component({
    selector: 'tree-ancestors',
    template:`
<div class="tree" *ngIf="_person">
	<ul>
		<li>
			<a [routerLink]="['AncestorTree', {id:_personId}]" [ngClass]="_person | personGender | slice:19 | lowercase">
        <div class="name">
          {{_person | personName}}
        </div>
      ...
      <ul *ngIf="_parentsOfPerson.length!=0">
        <li *ngFor="#relationship of _parentsOfPerson">
          <tree-ancestors [personId]="relationship.person1.resource|slice:1"></tree-ancestors>
        </li>
      </ul>
		</li>
	</ul>
...
directives:[TreeAncestorsComponent,ROUTER_DIRECTIVES]
})
export class TreeAncestorsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._dataProviderService.getRelationships().then(relationships=>{
      this._parentsOfPerson=relationships.filter(
        relationship=>{
          return relationship.type=="http://gedcomx.org/ParentChild" && relationship.person2.resource.substr(1)==this._personId;
        }
    )});
  }
}
